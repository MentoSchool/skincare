...
public class ApiMain
{
	...
	private static final String URI_LIFE_UV = SERVICE_URI_LIFE + "getUltrvLifeList";//자외선
	private static final String URI_LIFE_DS = SERVICE_URI_LIFE + "getDsplsLifeList";//불퀘지수
	private static final String URI_LIFE_SE = SERVICE_URI_LIFE + "getSensorytemLifeList";//체감온도
	private static final String URI_LIFE_DU = SERVICE_URI_LIFE + "getDustLifeList";//황사영향지수
	private static final String URI_LIFE_AIR = SERVICE_URI_LIFE + "getAirpollutionLifeList";//대기오염지수
	private static final String URI_LIFE_INF = SERVICE_URI_LIFE + "getInflWhoList";//감기지수
		...
		protected WrapperResult doInBackground(HttpGet... params)
		{
			...
			try
			{
				...
				if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK)
				{
					...
					return new WrapperResult().setObj(AbsResponse.fromXML(responseToString, UltraViolet.class)).setXml(responseToString);
		...
		protected void onPostExecute(WrapperResult result)
		{
			try
			{
				if (result != null)
				{
					...
					model.setToday(((UltraViolet) result.getObj()).getBody().getIndexModel().getToday());
					model.setDate(((UltraViolet) result.getObj()).getBody().getIndexModel().getDate());
					model.setH12(((UltraViolet) result.getObj()).getBody().getIndexModel().getH12());

					vo.setToday_UltraViolet(((UltraViolet) result.getObj()).getBody().getIndexModel().getToday());
					...
					Log.v("dd", "자외선::" + (prettyPrint(result.getXml()).toString().concat(((UltraViolet) result.getObj()).getBody().getIndexModel().getToday())));
					Log.v("dd", "열지수::" + (prettyPrint(result.getXml()).toString().concat(((UltraViolet) result.getObj()).getBody().getIndexModel().getH3())));
